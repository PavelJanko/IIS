    ...
    <div class="card">
        ...
        <div class="card-body py-5">
            <form method="POST" action="{{ route('devices.store') }}">
                ...
                @component('components.form-group', ['format' => 'text', 'name' => 'type', 'mandatory' => true]) Typ @endcomponent
                @component('components.form-group', ['format' => 'text', 'name' => 'manufacturer', 'mandatory' => true]) Výrobce @endcomponent
                    ...
                    <div class="col-md-6">
                        <select id="keeper_id" class="form-control{{ $errors->has('keeper_id') ? ' is-invalid' : '' }}" name="keeper_id" required>
                            @foreach($employees as $keeper)
                                <option value="{{ $keeper->id }}"{{ old('keeper_id') == $keeper->id ? ' selected' : '' }}>{{ $keeper->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
